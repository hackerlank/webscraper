    ...
    request({ url: hotelUrl, method: 'GET', gzip: true }, function (err, resp, hbody) {
        ...
        request({ har: har, gzip: true }, function (err, resp, body) {
            ...
            offers.forEach(function (h, index, array) {
                ...
                // fs.writeFileSync('hbody.html', hbody);
                // console.log(roomTypeCode);
                // var roomSize = hbody.match('var roomsAndRatePlans.*"roomTypeCode":"' + roomTypeCode + '","name":"[\\w\\s\(\)-]+","description":\\["<strong>[\\w\\s]+</strong><br />\\s[\\w\\s-]+\\((\\d+)')[1] + ' square meter';
                // console.log(bedtype);
                ...
                setTimeout(function () {
                    async.mapLimit(offsets, 1, function (offset, callback) {
                        ...
                        // console.log(urlComposer(offset).chkin);
                        ...
                        request({ har: thar, gzip: true }, function (err, resp, ibody) {
                            ...
                            // fs.writeFileSync('ibody.json', ibody);
                            if (!iOffers) {
                                row.push('N/A');
                                setTimeout(function () {
                                    callback();
