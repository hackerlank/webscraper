...
var excel = new Excel('yellowpage.xlsx', ["Bussiness Name", "Bussiness Type", "Contact Number", "Postal Address", "Email"], "result");
...
for (var i = 0; i < 100; i++) {
  (function () {
    ...
    var url = 'http://www.yellowpages.com.au/search/listings?clue=HariDressing&locationClue=Australia&pageNumber=' + k + '&referredBy=UNKNOWN&eventType=pagination';
    ...
    request({url: url, jar: j, headers: headers}, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        var types = $('.listing-heading a').map(function (index, element) {
          return $(this).text();
        }).get();

        var numbers = $('.contact-text').map(function (index, element) {
          return $(this).text();
        }).get();

        var address = $('.listing-address').map(function (index, element) {
          return $(this).text();
        }).get();

        var email = $('.contact-email').map(function (index, element) {
            ...
            numbers.forEach(function (numbers, numbersindex, numbersarr) {
              address.forEach(function (address, addressindex, addressarr) {
                email.forEach(function (email, emailindex, emailarr) {
                  row.push(name, type, numbers, address, email);
                  excel.appendRow(row);
